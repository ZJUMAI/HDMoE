...
def main(args):
    ...
    results_dir = "./result/{dataset}/[{model}]-[{fusion}]-[{alpha}]-[{time}]".format(
    ...
    for fold in range(5):
        ...
        dataset = Generic_MIL_Survival_Dataset(
            csv_path="./csv/tcga_blca_all_clean/%s_all_clean.csv" % (args.dataset),
            modal=args.modal,
            OOM=args.OOM,
            apply_sig=True,
        ...
        if args.model == "hdmoe":
            from models.hdmoe.network import HDMoE
            from models.hdmoe.engine import Engine
