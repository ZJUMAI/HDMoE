...
class Engine(object):
    ...
    def train(self, data_loader, model, criterion, optimizer):
        ...
        for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c) in enumerate(dataloader):
            ...
            f_list, moe_score, moe_loss, hazards, S, logits = model(x_path=data_WSI, x_omic1=data_omic1, x_omic2=data_omic2,
            ...
            loss_dis1 = criterion[1](f_list[2][:1], f_list[3]) + criterion[1](f_list[2][1:], f_list[4])
            loss_dis2 = criterion[2](f_list[0][:1], f_list[0][1:]) + criterion[2](f_list[1][:1], f_list[1][1:]) + \
                       criterion[2](f_list[2][:1], f_list[2][1:])

            loss_moe = moe_loss[0] + moe_loss[1] + moe_loss[2]
            loss = loss_sur  + self.args.alpha * (loss_dis1 + loss_dis2) + 0.01 * loss_moe
            print('loss',loss)
    ...
    def validate(self, data_loader, model, criterion):
        ...
        for batch_idx, (data_WSI, data_omic1, data_omic2, data_omic3, data_omic4, data_omic5, data_omic6, label, event_time, c) in enumerate(dataloader):
            ...
            # survival loss + sim loss + moe loss
            loss_sur = criterion[0](hazards=hazards, S=S, Y=label, c=c)
            loss_dis1 = criterion[1](f_list[2][:1], f_list[3]) + criterion[1](f_list[2][1:], f_list[4])
            loss_dis2 = criterion[2](f_list[0][:1], f_list[0][1:]) + criterion[2](f_list[1][:1], f_list[1][1:]) + \
                        criterion[2](f_list[2][:1], f_list[2][1:])
