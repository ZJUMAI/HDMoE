...
def define_loss(args):
    if args.loss == "ce_surv":
        ...
        print('########### ', "nll_surv_kl")
        loss = [NLLSurvLoss(alpha=0.0), KLLoss(), InverseKL_Loss()]
    elif args.loss == "nll_surv_mse":
        print('########### ', "nll_surv_mse")
        loss = [NLLSurvLoss(alpha=0.0), nn.MSELoss(), InverseMSE_Loss()]
    elif args.loss == "nll_surv_l1":
        print('########### ', "nll_surv_l1")
        loss = [NLLSurvLoss(alpha=0.0), nn.L1Loss(), InverseL1_Loss()]
    elif args.loss == "nll_surv_cos":
        print('########### ', "nll_surv_cos")
        loss = [NLLSurvLoss(alpha=0.0), CosineLoss(), CosineLoss1()]
...
class InverseKL_Loss(object):
    def __call__(self, y, y_hat):
        return 1 / (F.kl_div(y_hat.softmax(dim=-1).log(), y.softmax(dim=-1), reduction="sum") + 1e-6)
...
class InverseL1_Loss(object):
    def __call__(self, y,y_hat):
        return 1 / (F.l1_loss(y,y_hat) + 1e-6)
